...
class PhotoQqc(models.Model):
    archive = models.ForeignKey(ArchiveQqc, related_name='photos', on_delete=models.CASCADE)
...
class ConnectionQqc(models.Model):
    title = models.CharField(max_length=255)
...
class GovernanceQqc(models.Model):
...
class ResolutionQqc(models.Model):
...
class ProvisionQqc(models.Model):
