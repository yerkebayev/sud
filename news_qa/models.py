...
# Photo Model
class PhotoQa(models.Model):
    gallery = models.ForeignKey(GalleryQa, related_name='photos', on_delete=models.CASCADE)
    image = models.ImageField(upload_to='gallery_photos/')
    uploaded_at = models.DateTimeField(auto_now_add=True)


class ConnectionQa(models.Model):
    title = models.CharField(max_length=255)
...
class GovernanceQa(models.Model):
...
class ResolutionQa(models.Model):
...
class ProvisionQa(models.Model):
